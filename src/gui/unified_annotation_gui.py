...
from gui.experiment_adapters import ExperimentDataAdapter, QueryResult, get_adapter
from gui.chunk_widgets import create_chunk_widget, create_chunks_summary
...
class UnifiedAnnotationGUI:
    ...
    def _load_annotations_cache(self):
    ...
    def _create_widgets(self):
        ...
        self.prev_button = widgets.Button(description='< Prev', button_style='info', layout=widgets.Layout(width='80px'))
        self.next_button = widgets.Button(description='Next >', button_style='info', layout=widgets.Layout(width='80px'))
        ...
        # Validation buttons
        self.pass_button = widgets.Button(description='Pass', button_style='success', layout=widgets.Layout(width='80px'))
        self.fail_button = widgets.Button(description='Fail', button_style='danger', layout=widgets.Layout(width='80px'))
    ...
    def _on_jump_unvalidated(self, btn):
    ...
    def _on_open_coding_change(self, change):
        self._save_annotation('open_coding', change['new'])

    def _on_axial_coding_change(self, change):
    ...
    def _save_annotation(self, key: str, value: Any):
    ...
    def _update_query_display(self, query: QueryResult):
        ...
        refs_html = ''
        if doc_refs:
            refs_list = ''.join([f'<li><a href="{url}" target="_blank" style="color: #1565c0;">{url}</a></li>' for url in doc_refs])
            refs_html = f'''
    ...
    def _update_response_display(self, query: QueryResult):
    ...
    def _update_validation_status(self):
        ...
        if validation == 'pass':
            ...
            self.validation_status.value = '<span style="color: gray; font-size: 14px;">Not validated</span>'

    def _update_coding_inputs(self, query: QueryResult):
    ...
    def _update_chunks_display(self, query: QueryResult):
        """Update chunks display with auto-relevance."""
        if not query.retrieved_chunks:
            self.summary_container.children = []
            self.chunks_container.children = [
                widgets.HTML(value='<div style="padding: 20px; text-align: center; color: #666;"><i>No chunks retrieved</i></div>')
            ]
            return
    ...
    def _create_chunk_widget(
        ...
        for ra in relevance_list:
            if ra.get('chunk_index') == chunk_idx:
                is_relevant = ra.get('is_relevant', False)
                has_annotation = True
                break

        # Auto-populate if no annotation exists
        if not has_annotation:
            source_url = chunk.get('source_url', '')
            if source_url and any(source_url in ref or ref in source_url for ref in doc_refs):
                is_relevant = True
                # Save auto-populated relevance
        ...
        # Callback for relevance changes
        def on_relevance_change(new_value: bool):
            ...
            found = False
            for ra in ann['relevance_annotations']:
                if ra.get('chunk_index') == chunk_idx:
                    ra['is_relevant'] = new_value
                    found = True
                    break
            if not found:
                ann['relevance_annotations'].append({
                    'chunk_index': chunk_idx,
                    'is_relevant': new_value
                })

            # If marking as relevant, add source_url to doc_references
            if new_value and self.queries_json_path:
                ...
                if source_url:
                    ...
                    if added:
                        # Update local cache
                        if query_id in self._ground_truth_cache:
            ...
            # Refresh the chunks display to update metrics
            query = self.queries[self.current_index]
            self._update_chunks_display(query)
    ...
    def display(self):
        ...
        chunks_header = widgets.HBox([
            widgets.HTML(value='<h3 style="margin: 0; color: #1976d2; font-size: 18px; font-weight: bold;">Retrieved Chunks</h3>'),
            self.collapse_all_button
        ], layout=widgets.Layout(justify_content='space-between', align_items='center', margin='0 0 10px 0'))
        ...
        tabs = widgets.Tab()
        tabs.children = [chunks_section, query_response_section]
        tabs.set_title(0, f'Chunks ({len(self.queries[self.current_index].retrieved_chunks)})')
        tabs.set_title(1, 'Query & Validation')

        # Update tab title when switching queries
        def update_on_query_change():
            query = self.queries[self.current_index]
            tabs.set_title(0, f'Chunks ({len(query.retrieved_chunks)})')

        # Hook into query changes
        original_update = self._update_display
        def enhanced_update():
            original_update()
            update_on_query_change()
        self._update_display = enhanced_update
