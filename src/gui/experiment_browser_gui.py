...
class ExperimentBrowserGUI:
    ...
    def __init__(self, outputs_dir: Optional[Path] = None):
        ...
        self.selected_experiment: Optional[Dict[str, Any]] = None
        self._annotation_gui = None
    ...
    def _update_experiment_list(self):
        ...
        self.count_label.value = f'<span style="color: #666;">Found {len(experiments)} experiments</span>'
    ...
    def _on_experiment_select(self, change):
    ...
    def _update_preview(self):
        ...
        if exp.get('type') == 'retrieval':
            rows.append(f'<tr><td><b>Retriever:</b></td><td>{exp.get("retriever_type", "unknown")}</td></tr>')
            rows.append(f'<tr><td><b>Chunking:</b></td><td>{"Yes" if exp.get("chunking") else "No"}</td></tr>')
        elif exp.get('type') == 'generation':
            rows.append(f'<tr><td><b>Retrieval Source:</b></td><td>{exp.get("retrieval_reference", "N/A")}</td></tr>')
            rows.append(f'<tr><td><b>System Prompt:</b></td><td>{exp.get("system_prompt_version", "unknown")}</td></tr>')
        ...
        rows.append(f'<tr><td><b>Path:</b></td><td style="font-size: 13px; word-break: break-all;">{path_str}</td></tr>')
...
def browse_experiments(outputs_dir: Optional[Path] = None) -> ExperimentBrowserGUI:
    """
    ...
    Example:
        ...
        # Select an experiment in the GUI, then:
        >>> gui = browser.get_annotation_gui()
    """
