...
def scrape_tribe_exclusivity_table(
    url: str,
    table_index: int = 1
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Required column not found in table headers: {e}")
...
def enrich_card_with_tribe(
    ...
    try:
        ...
        matching_enum = next(
            t for t in TribeExclusivity 
            if t.value == tribe_name_str
        )
...
def enrich_cards_with_tribes(
    ...
    try:
        ...
        items_tribe_lookup = scrape_tribe_exclusivity_table(
            items_url,
            table_index=0
        )
