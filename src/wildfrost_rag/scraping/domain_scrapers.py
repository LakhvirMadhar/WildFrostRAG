...
from wildfrost_rag.data_processing.charms import parse_charms_page, CharmInfo
...
from wildfrost_rag.data_processing.bells import parse_bells_page, BellInfo
...
from wildfrost_rag.data_processing.shades import parse_shades_page, SummonInfo
from wildfrost_rag.data_processing.cards import CardInfo
from wildfrost_rag.scraping.wiki_scraper import scrape_wiki_page, load_cached_html
...
async def scrape_stats() -> tuple[list[StatInfo], PageUrls]:
    ...
    stats = parse_stats_page(html, base_url=get_settings().scraping.wildfrost_wiki_base_url)
...
async def scrape_charms() -> tuple[list[CharmInfo], PageUrls]:
    ...
    charms = parse_charms_page(html, base_url=get_settings().scraping.wildfrost_wiki_base_url)
...
async def scrape_bells() -> tuple[list[BellInfo], PageUrls]:
    ...
    bells = parse_bells_page(html, base_url=get_settings().scraping.wildfrost_wiki_base_url)
