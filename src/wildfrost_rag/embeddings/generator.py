...
class EmbeddingGenerator:
    ...
    def generate_embeddings(self, texts: list[str] | list[Document]) -> np.ndarray:
        ...
        if texts and isinstance(texts[0], Document):
            text_list = [doc.page_content for doc in texts if isinstance(doc, Document)]
            logger.debug(f"Extracting text from {len(texts)} Document objects")
        else:
            text_list = [t for t in texts if isinstance(t, str)]
        ...
        embeddings: np.ndarray = self.model.encode(text_list)
...
def generate_embeddings(
    texts: list[str] | list[Document], model: SentenceTransformer
) -> np.ndarray:
