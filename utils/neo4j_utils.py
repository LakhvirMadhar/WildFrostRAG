...
def create_card_tribe_relationships(tx, cards_data):
    ...
    for card_dict in cards_data:
        ...
        if 'tribe_exclusivity' in card_dict and card_dict['tribe_exclusivity'] is not None:
            ...
            # Note: This is a robust way to convert the string value back to the enum member
            try:
                exclusivity_enum_member = next(t for t in TribeExclusivity if t.value == card_dict['tribe_exclusivity'])
            except StopIteration:
                # This handles potential data mismatches where the string value doesn't match an enum member
                continue

            # Use the get_tribes() method to get the list of tribe names
            # This method already correctly handles both exclusive and universal tribes
