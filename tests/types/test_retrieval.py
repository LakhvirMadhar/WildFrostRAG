...
class TestRetrievedChunk:
    ...
    def test_from_raw_bm25(self, raw_bm25_chunk):
        ...
        assert chunk.cypher_result["source_file"] == "data/structured_outputs/companions/Bombom.html"
    ...
    def test_from_raw_text2cypher(self, raw_text2cypher_chunk):
    ...
    def test_from_raw_hybrid_doc_source_url_fallback(self, raw_hybrid_chunk):
    ...
    def test_source_url_prefers_source_url_over_doc_source_url(self):
    ...
    def test_to_dict_json_serializable(self, raw_bm25_chunk):
        chunk = RetrievedChunk.from_raw_retriever_dict(raw_bm25_chunk)
        d = chunk.to_dict()
    ...
    def test_roundtrip_to_dict_from_dict(self, raw_bm25_chunk):
    ...
    def test_roundtrip_text2cypher(self, raw_text2cypher_chunk):
...
class TestQueryResult:
    ...
    def test_to_dict_includes_cypher_execution(self, raw_bm25_chunk):
        chunk = RetrievedChunk.from_raw_retriever_dict(raw_bm25_chunk)
        qr = QueryResult(
            query_id=1,
    ...
    def test_bm25_cypher_execution_null_query(self, raw_bm25_chunk):
        """BM25 results have cypher_execution with null cypher_query."""
        chunk = RetrievedChunk.from_raw_retriever_dict(raw_bm25_chunk)
        ce = CypherExecution(cypher_query=None, cypher_execution_status="success", cypher_error_message=None)
        qr = QueryResult(
            query_id=1, query="What does Bombom do?",
            cypher_execution=ce, retrieved_chunks=[chunk],
        )
        d = qr.to_dict()
        assert d["cypher_execution"]["cypher_query"] is None
        assert d["cypher_execution"]["cypher_execution_status"] == "success"

    def test_text2cypher_cypher_execution_populated(self, raw_text2cypher_chunk):
        ...
        qr = QueryResult(
            query_id=1, query="Find Foxee",
            cypher_execution=ce, retrieved_chunks=[chunk],
        )
        d = qr.to_dict()
        assert d["cypher_execution"]["cypher_query"] == "MATCH (c:Card) WHERE c.card_name = 'Foxee' RETURN c.card_name"

    def test_text2cypher_failure_zero_rows(self):
        ...
        qr = QueryResult(
            query_id=2, query="What tribe is Sneezle in?",
            cypher_execution=ce, retrieved_chunks=[],
        )
        d = qr.to_dict()
        assert d["cypher_execution"]["cypher_execution_status"] == "failed"
        assert d["cypher_execution"]["cypher_error_message"] == "RelationshipType BELONGS_TO does not exist"
        assert d["retrieved_chunks"] == []

    def test_roundtrip(self, raw_bm25_chunk, raw_text2cypher_chunk):
        chunks = [
            RetrievedChunk.from_raw_retriever_dict(raw_bm25_chunk),
            RetrievedChunk.from_raw_retriever_dict(raw_text2cypher_chunk),
        ]
        ce = CypherExecution(cypher_query="RETURN 1", cypher_execution_status="success", cypher_error_message=None)
    ...
    def test_json_serializable(self, raw_bm25_chunk):
        chunk = RetrievedChunk.from_raw_retriever_dict(raw_bm25_chunk)
