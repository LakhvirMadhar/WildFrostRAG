...
async def main():
    ...
    for i, retriever_config in enumerate(retrievers):
        print(f"\n[{i+1}/{len(retrievers)}] Running {retriever_config['type']}...", end='')
        if 'embedder' in retriever_config:
...
def experiment_exists(run_num: int, retriever_config: dict) -> bool:
...
async def run_retriever_subprocess(run_num: int, chunking: bool, retriever_config: dict):
    """Call evaluate_retrievers.py as subprocess"""
    cmd = [
    ...
    # Add embedder for vector-based retrievers
    if 'embedder' in retriever_config:
        cmd.extend(["--embedder", retriever_config['embedder']])

    # Add text2cypher prompt if specified
    if retriever_config['type'] == 'text2cypher':
        prompt = retriever_config.get('prompt', 'TEXT2CYPHER_PROMPT_V1')
        cmd.extend(["--text2cypher-prompt", prompt])
